...
// Get the lowest frequented char
	int getlowest(chartableentry[] chartable) {
		...
		while (chartable[min].count == 0) {
			min++;
			if (min == chartable.length)
				return (-1);
		}
		for (int i = min; i < chartable.length; i++) {
			if ((chartable[min].count > chartable[i].count) && (chartable[i].count > 0))
				min = i;
		}
		return (min);
	}
...
//	System.out.println("bt(tree,"+i+","+code+")");
		if (tree[i].edge1 >= 0) {
			...
			buildtranstable(tree, Math.abs(tree[i].edge1), code + 0);
		if (tree[i].edge2 >= 0) {
			transtable[tree[i].edge2] = code + "1";
		} else
			buildtranstable(tree, Math.abs(tree[i].edge2), code + 1);
	}
...
// dataout will be filled up exactly (-> may not be longer than code expected!)
	public void uncompress(byte[] datain, byte dataout[], int[] treein) {
		...
		String s = "";
		for (i = 0; i < datain.length; i++)
			s += dual(makeunsigned(datain[i]));
		uncode(s, dataout, treein);
	}

	/*
	 * // Test and demonstration method. public static void main(String[] args) {
	 * Huffman h = new Huffman(); System.out.println();
	 ...
	 * System.out.
	 * println("of the GNU Library Public License 2.00 (see COPYING.LIB).\n\n"); if
	 * ((args.length!=1) && (args.length!=2)) {
	 * System.out.println("huffman <teststring> [d]"); System.out.
	 * println("If you specify a second argument, you will get debugging output");
	 * return; } int i,l2,ts; int l=args[0].length(); int huftree[] = new
	 * int[ALPHABETSIZE]; byte dataout2[] = new byte[l]; byte datain[] = new
	 * byte[l]; byte dataout[] = new byte[l];
	 * 
	 * for (i=0;i<l;i++) datain[i]=(byte) args[0].charAt(i);
	 * 
	 * if (args.length==2) { System.out.println("Input (ASCII Code):"); for
	 * (i=0;i<l;i++) System.out.print(makeunsigned(datain[i])+" "); }
	 * 
	 * l=datain.length; l2=h.compress(datain,dataout,huftree);
	 * ts=h.treesize(huftree); if (args.length==2) {
	 * System.out.println("\nCompressed (New Code):"); for (i=0;i<l2;i++)
	 * System.out.print((byte) dataout[i]+" "); }
	 * 
	 * h.uncompress(dataout,dataout2,huftree);
	 * 
	 * if (args.length==2) { System.out.println("\nUncompressed:"); for
	 * (i=0;i<dataout.length;i++) System.out.print((byte) dataout2[i]+" ");
	 * System.out.println(); System.out.println();
	 * System.out.println("Tree length: "+ts); }
	 * 
	 * System.out.println("Ratio (without tree):   1:"+(float) l/l2);
	 * System.out.print ("Real ratio (with tree): 1:"+(float) l/(l2+ts)); if
	 * (l/(l2+ts)<1) System.out.println(" => INFLATION!"); else
	 * System.out.println(); }
	 */
}